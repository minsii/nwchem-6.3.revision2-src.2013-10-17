 ...
 *  necessory.  
 */
static enum PendBufOrderingRule _pbufOrder = PUTACC_SPLIT_ORDER;
...
/*       fflush(stdout); */
      ...
      armci_pbuf_start_get(rptr,lptr,bytes,msginfo->from,index);
...
/*     fflush(stdout); */
    if(sizeof(request_header_t)+msginfo->dscrlen <= IMM_BUF_LEN) {
      ...
      armci_pbuf_start_get(msginfo->tag.data_ptr,buffer,msginfo->tag.data_len,
			   msginfo->from, index);

...
void ARMCIX_AllFence ()
{
  ...
  /* it turns out that __connection[peer].active is only touched by NB operations
     so this version of AllFence is not going to do the right thing. */

  /* everyone starts with a different offset to prevent hammering node 0 at the start */
  for (peer = rank; peer < size; peer++)
  {
    //if (__connection[peer].active > 0) ARMCIX_Fence(peer);
    ARMCIX_Fence(peer);
  }
